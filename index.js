...
exports.handler = function( event, context ) {
    ...
    if (event.request.type === "LaunchRequest") {
        say = "Welcome to Alexa Stocks! What stock would like to know the price of?";
         var response = {
        outputSpeech: {
            type: "SSML",
    ...
    Respond(  // Respond with normal speech only
        ...
        if (IntentName === "ISeeIntent") {
            ...
            if(event.request.intent.slots.Animal.value) {
                ...
                //sessionAttributes.myList.push(myColor + " " + myAnimal);
                	if (myAnimal == 'Google'){
                		 googleStocks(['GOOG'], function(error, data) {
						  say = "The stock price for " + myAnimal + " is " + data[0]['l'] + "dollars";
						  var response = {
						        outputSpeech: {
						            type: "SSML",
						            ssml: "<speak>" + say + "</speak>"
		                ...
		                googleStocks(['AMZN'], function(error, data) {
						  say = "The stock price for " + myAnimal + "is " + data[0]['l'] + "dollars";
						  var response = {
						        outputSpeech: {
						            type: "SSML",
